...
func (self *FormatEXT2)Set_parent_child_relation(parent types.IndexNode, child types.IndexNode) {
...
func (self *FormatEXT2)Put_in_dir(dir types.IndexNode, trgt_inode InodeTemplate,name [12]string) (types.IndexNode){ 
...
func (self *FormatEXT2)create_ptr_block_recursive_and_append(pointer_block *types.PointerBlock, level int32, trgt_inode InodeTemplate,name [12]string) (types.IndexNode){
	if level != 0 {
		for n,ptr := range pointer_block.B_pointers().Get(){
			if ptr != -1 { continue }
...
func (self *FormatEXT2)case2_ptr_block(pointer_block *types.PointerBlock, level int32, trgt_inode InodeTemplate,name [12]string) (types.IndexNode,*PntrBlockAvailable){
...
func (self *FormatEXT2)try_put_in_new_dir_block_in_existing_ptr(dir types.IndexNode, trgt_inode InodeTemplate,name [12]string) (types.IndexNode){
...
func (self *FormatEXT2)case1_ptr_blck(pointer_block *types.PointerBlock, level int32, trgt_inode InodeTemplate,name [12]string) types.IndexNode{
	if level != 0 {
		for _,ptr := range pointer_block.B_pointers().Get(){
			if ptr == -1 { continue }
			ptr_block := types.CreatePointerBlock(self.super_service,ptr)
...
func (self *FormatEXT2)try_put_in_existing_dir_block(dir types.IndexNode, trgt_inode InodeTemplate,name [12]string) types.IndexNode{
	for n,ptr := range dir.I_block().Get(){
		if ptr == -1 { continue }
		if n >= 13{
			ptr_block := types.CreatePointerBlock(self.super_service,ptr)
...
func (self *FormatEXT2)Get_shallow_tree_of_childs(dir types.IndexNode) []types.Content {
...
func (self *FormatEXT2)strict_shallow_tree_from_ptr_blck(pointer_block *types.PointerBlock,level int32,all_content *[]types.Content) {
	if level != 0 {
		for _,ptr := range pointer_block.B_pointers().Get(){
			if ptr == -1 { continue }
			ptr_block := types.CreatePointerBlock(self.super_service,ptr)
...
func (self *FormatEXT2)Get_strict_shallow_tree_of_childs(dir types.IndexNode) []types.Content {
	all_content := make([]types.Content,0,10)
	for n,ptr := range dir.I_block().Get(){
		if ptr == -1 { continue }
		if n >= 13{
...
func (self *FormatEXT2) User_allowed_actions_with_ownership(user int32, grp int32,inode *types.IndexNode)PermisionCase{
...
func (self *FormatEXT2) Wrap_indx_in_template(index int32)InodeTemplate{
	return InodeTemplate{
		index:  index,
		sketch: types.IndexNodeHolder{},
	}
}
func (self *FormatEXT2) Wrap_holder_in_template(template types.IndexNodeHolder)InodeTemplate{
	return InodeTemplate{
		index:  -1,
		sketch: template,
	}
...
func (self *FormatEXT2)erase_inode_content_from_ptr_blck(pointer_block *types.PointerBlock,level int32,inode_type utiles.InodeType,usr_id int32,usr_grp_id int32,time types.TimeHolder)bool {
	can_erase_self:=true
	if level != 0 {
		for i,ptr := range pointer_block.B_pointers().Get(){
			if ptr == -1 { continue }
			ptr_block := types.CreatePointerBlock(self.super_service,ptr)
			can_erase := self.erase_inode_content_from_ptr_blck(&ptr_block,level-1,inode_type,usr_id,usr_grp_id,time)
			if can_erase{
...
func (self *FormatEXT2)Erase_inode_content(inode types.IndexNode, inode_type utiles.InodeType,usr_id int32,usr_grp_id int32,time types.TimeHolder) bool {
	can_erase_self := true
	for n,ptr := range inode.I_block().Get(){
		if ptr == -1 { continue }
		if n >= 13{
			ptr_block := types.CreatePointerBlock(self.super_service,ptr)
			can_erase := self.erase_inode_content_from_ptr_blck(&ptr_block,int32(n)-13,inode_type,usr_id,usr_grp_id,time)
			if can_erase{
				bit := self.Block_section.Bit_no_for(ptr_block.Index)
				self.Block_bitmap.Erase(1,bit)
...
func (self *FormatEXT2)Remove_inode_if_possilbe(in_dir types.IndexNode,with_name [12]string,usr_id int32,usr_grp_id int32,time types.TimeHolder) bool {
	for n,ptr := range in_dir.I_block().Get(){
		if ptr == -1 { continue }
		if n >= 13{
			ptr_block := types.CreatePointerBlock(self.super_service,ptr)
			found := self.renove_inode_if_possilbe_in_ptr_block(&ptr_block,int32(n-13),with_name,usr_id,usr_grp_id,time)
			if found {
				for _,sub_ptr := range ptr_block.B_pointers().Get(){
...
func (self *FormatEXT2)renove_inode_if_possilbe_in_ptr_block(pointer_block *types.PointerBlock, level int32, name [12]string,usr_id int32,usr_grp_id int32,time types.TimeHolder) bool {
	if level != 0 {
		for i,ptr := range pointer_block.B_pointers().Get(){
			if ptr == -1 { continue }
			ptr_block := types.CreatePointerBlock(self.super_service,ptr)
			found := self.renove_inode_if_possilbe_in_ptr_block(&ptr_block,level-1,name,usr_id,usr_grp_id,time)
			if found {
				for _,sub_ptr := range ptr_block.B_pointers().Get(){
					if sub_ptr != -1{return true}
				}
...
func (self *FormatEXT2)Change_owner(dest_dir types.IndexNode,usr_id int32,usr_grp_id int32,time types.TimeHolder,new_usr_id int32,new_usr_grp_id int32,recursive bool) int {
	changed:=0
...
func (self *FormatEXT2)Change_ugo_permition(dest_dir types.IndexNode,time types.TimeHolder,recursive bool,new_perm [3]string) int {
	changed:=0
	dest_dir.I_mtime().Set(time)
